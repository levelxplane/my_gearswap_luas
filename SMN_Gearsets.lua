...
function get_sets()
    ...
    sets.avatar.perp = smn_gear.perpetuation
    ...
    -- Summoning Magic. This is a base set for max skill
    -- and blood pacts and we'll overwrite later as and when we need to
    sets.avatar.skill = smn_gear.summoning
    ...
    -- Physical damage
    sets.avatar.atk = set_combine(sets.avatar.skill, smn_gear.bloodpact)
    ...
    -- Magic Attack
    sets.avatar.mab = set_combine(sets.avatar.skill, smn_gear.bloodpact)

    sets.avatar.mb = set_combine(sets.avatar.mab, {hands="Glyphic Bracers +1"})

	-- Hybrid
    sets.avatar.hybrid = set_combine(sets.avatar.skill, smn_gear.bloodpact)
    ...
    sets.precast.casting = smn_gear.fastcast

    -- Summoning Magic Cast time - gear
    sets.precast.summoning = smn_gear.summoning
    ...
    -- this needs to get more blood pact delay reduction
    sets.precast.bp = smn_gear.summoning
    ...
    sets.midcast.casting = smn_gear.idle
    ...
    -- Cure Potency
    sets.midcast.cure = set_combine(sets.midcast.casting, smn_gear.healing)
